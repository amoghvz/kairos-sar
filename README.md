...
- **Foresight.** For any place, decades of satellite records answer three questions: how exposed is it to floods, wildfire, drought and subsidence; which months the risk peaks; and whether it is getting statistically worse (Mann-Kendall and OLS trend tests, real p-values). One button checks all four hazards and ranks them. You can ask questions about your own outlook in plain language and get an answer grounded in those exact numbers, print or save the whole thing as a report, and share a link that reopens it. Each outlook comes with a preparedness checklist and, for floods, a live accuracy check against the independent Global Flood Database.
